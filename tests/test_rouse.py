...
class TestModel(myTestCase):
    def setUp(self):
        ...
        self.model_tf = rouse.Model(5, 1, 1,use_tf=(True,0))

    def test_setup(self):
        for use_tf in [(True,0), (False,None)]:
            with self.assertRaises(ValueError):
                mod = rouse.Model(5, 1, -1, use_tf=use_tf)
            mod = rouse.Model(5, 1, 1, setup_dynamics=False, use_tf=use_tf)
            mod.F[-1] = [1, 0, 0]
            mod.update_dynamics()
            ...
            mod = rouse.Model(1, use_tf=use_tf)
            self.assert_array_equal(mod.A, np.array([[0.0]]))
    ...
    def test_operators(self):
        ...
        self.assertTrue(
            self.model != self.model_nosetup
        )  # we have != as mixin for == :)
        ...
        self.assertEqual(
            repr(rouse.Model(5, 1, 1, add_bonds=[(2, 4), (1, 3, 0.5)])),
    ...
    def test_check_dynamics(self):
        ...
        self.model_tf.check_dynamics(dt=1)
    ...
    def test_propagate(self):
        for m in [self.model,self.model_tf]:
            _, C0 = m.steady_state()
            M0 = np.array(3 * [np.linspace(0, 1, m.N)]).T
            M1, C1 = m.propagate(M0, C0, 0.1)

            self.assertTrue(np.allclose(M1, M0, atol=1))
            self.assertTrue(np.allclose(C1, C0, atol=1))

            self.assertTrue(np.allclose(M1, m.propagate_M(M0)))
            self.assertTrue(np.allclose(C1, m.propagate_C(C0)))

            m.D = 0
            M2, C2 = m.propagate(M0, np.zeros_like(C0), 0.1)

            self.assertEqual(m._dynamics["D"], m.D)
    ...
    def test_contact_frequency(self):
        ...
        self.assert_array_equal(
            np.round(np.diagonal(hic, 1), 10), np.round(np.diagonal(hic, -1), 10)
        )
